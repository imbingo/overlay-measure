...
- 批量导入表保留原有简洁结构，并新增“配对”列：双图模式明确显示已配对组数或未配对数量；单图模式显示无需配对；状态统一显示就绪、未导入或需检查。
- 上下层数量不一致时继续禁止批量运行，并可直接从表格确认是哪一个 Mark 需要处理。
- 项目设计规则明确：功能图标必须简约、低对比、辅助文字理解；不得使用大面积黑色、拟物化或抢占按钮文字层级的图标。顶部命令继续保持 V2.2.4 的文字/简洁符号风格。

## V2.4.5 批量图像预览切换

- 批量模式下，画布工具栏会在当前 Mark 有两张以上图片时显示紧凑的上一张/下一张预览控件。
- 导入完成后即可逐张检查 ROI 是否覆盖目标，不必等到运行测量程序；切换只改变画布预览，不改动批量队列、ROI、Recipe 或既有结果。
- 双图模式按相同序号同步切换上/下层；控件显示各层当前序号和总数，导入数量不一致时可在运行前发现。

## V2.4.4 批量多文件夹与文件对话框记忆

- 批量导入新增“选择多个文件夹”：在专用目录树中可用 `Ctrl`/`Shift` 多选并追加多个文件夹的图像，继续复用既有递归、自然排序、去重和批量配对逻辑。
- 图像导入、批量图片、批量文件夹和报告导出分别记住本机上一次位置；重新打开时回到该目录，并预选上次单文件或在多文件夹窗口中保留上次文件夹列表。
- 对话框位置数据存放在用户本机应用数据目录，不写入 Recipe，不影响其他操作者或安装升级。

## V2.4.3 独立图像查看器与配方入口整理

- “查看图像”会打开可自由调整大小的独立只读查看器；单图显示一个画布，双图并排显示上下层画布，并保留当前 ROI、识别轮廓和几何标注。
- 查看器支持滚轮缩放、中键或空格拖动、适应窗口和 `1:1` 实际像素查看，不再使用会遮挡图像的局部放大镜。
- 最近文件移动到顶栏，并放在当前配方左侧；“配方管理”和“保存配方”收进当前配方菜单，减少顶栏重复操作入口。
- 修复切换单图/双图模式后，新导入下层图像可能被错误写入上层的状态同步问题。

## V2.4.2 直接图像缩放

- 移除局部放大镜，改用图像工作区自身的直接缩放，避免局部镜片遮挡 ROI、轮廓和标注。
- 工具栏增加 `1:1` 实际像素查看；缩放档位扩展至 800%。单图和双图均支持滚轮缩放、中键或空格拖动画布，以及一键恢复适应窗口。

## V2.4.1 图像查看与最近文件

- 新增可开关的局部放大镜：在全图视图上移动鼠标即可查看局部 3 倍放大，不改变缩放、ROI、诊断或量测结果；单图和双图画布均支持。
- 新增“最近文件”快速导入菜单：记录最近使用的上层/单图与下层文件，显示父文件夹和文件名；重启后仍可用，失效路径自动跳过，并可一键清除记录。
- 最近文件与拖放、常规导入共用同一导入链路，因此不会绕过旧结果失效、批量退出和图像来源追溯规则。

## V2.4.0 自动精测语义、椭圆轴与图像诊断

- 自动识别仍先进行全图候选搜索，但每个候选的精测现在遵循右侧当前 ROI 类型：卡尺圆使用卡尺圆；圆、圆环、椭圆、区域中心和稳健轮廓中心复用对应 ROI 的亚像素测量路径；矩形继续使用四边卡尺精测。
- 选择“椭圆区域（计算圆度）”后，自动孔阵列候选按椭圆拟合输出长轴、短轴、等效直径与椭圆圆度；椭圆圆度仍定义为 `(长轴 - 短轴) / 2`。
- 画布按拟合角度绘制椭圆轮廓，并叠加绿色长轴、青色短轴及物理长度，便于确认圆度结果对应的实际要素。
- “更多”中新增工程模式专用“图像诊断与边缘分析”：在图像上拖动剖面线即可查看原始灰度曲线、长度、最小/最大/平均灰度和 PV；不改变显示增强、ROI 或测量结果。
- 顶部命令恢复以文字和简洁符号为主的 V2.2.4 风格，去除较突出的系统图标；折叠栏继续采用简单方向箭头。

## V2.3.0 性能与孔阵列识别

- 性能改动 1：手动多 ROI 的通用亚像素边缘提取默认均匀限采样为最多 2,000 点，避免大 ROI 或高噪声图无限增加拟合、内存和 UI 绘制成本；RANSAC 和边缘扫描支持在内部循环响应取消。
- 非激活 ROI 默认只绘制识别中心和编号；需要完整拟合轮廓时可开启诊断显示，避免平移、缩放时为大量 ROI 重建复杂路径。
- 识别改动 2：自动孔阵列识别增加局部自适应阈值以适应照明不均；候选去重后保留。
- 自动识别的轮廓数、候选数与时间预算改为 Recipe 参数，默认分别为每个阈值 256、每张图 128 和 12 秒；不再在上下层汇总时静默只精测前 32 个候选，避免规则阵列被截半。
