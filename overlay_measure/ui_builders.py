...
class MainWindowBuilderMixin:
        ...
        def _build_ui(self):
            ...
            # Keep command buttons text-first. Native platform icons made this
            # compact engineering toolbar visually heavier than the V2.2 style.
            ...
            for btn in (self.recent_images_btn, self.load_recipe_btn):
            ...
            self.image_viewer_btn = QPushButton("查看图像")
            self.image_viewer_btn.setToolTip("在独立窗口中查看当前单图或双图画布")
            self.batch_preview_nav = QWidget()
            batch_preview_layout = QHBoxLayout(self.batch_preview_nav)
            batch_preview_layout.setContentsMargins(0, 0, 0, 0)
            batch_preview_layout.setSpacing(3)
            self.batch_preview_previous_btn = QToolButton()
            self.batch_preview_previous_btn.setText("‹")
            self.batch_preview_previous_btn.setToolTip("预览上一张批量图像")
            self.batch_preview_index_label = QLabel("批量预览 1/1")
            self.batch_preview_index_label.setObjectName("statusCaption")
            self.batch_preview_next_btn = QToolButton()
            self.batch_preview_next_btn.setText("›")
            self.batch_preview_next_btn.setToolTip("预览下一张批量图像")
            batch_preview_layout.addWidget(self.batch_preview_previous_btn)
            batch_preview_layout.addWidget(self.batch_preview_index_label)
            batch_preview_layout.addWidget(self.batch_preview_next_btn)
            self.batch_preview_nav.setVisible(False)
            ...
            self.import_images_btn.setToolButtonStyle(Qt.ToolButtonTextOnly)
            ...
            self.image_diagnostics_action = self.more_actions_menu.addAction("图像诊断与边缘分析")
            ...
            command_layout.addWidget(self.image_viewer_btn)
            command_layout.addWidget(self.batch_preview_nav)
        ...
        def _build_algo_tab(self) -> QWidget:
            ...
            diagnostics_section = CollapsibleSection("图像诊断与边缘分析", False)
            diagnostics_group = QGroupBox("原始灰度诊断")
            diagnostics_layout = QVBoxLayout(diagnostics_group)
            diagnostics_hint = QLabel("在当前图像上拖动一条线，查看原始灰度剖面与统计；不修改原图或测量结果。")
            diagnostics_hint.setWordWrap(True)
            diagnostics_hint.setObjectName("statusCaption")
            self.image_diagnostics_btn = QPushButton("打开灰度剖面")
            diagnostics_layout.addWidget(diagnostics_hint)
            diagnostics_layout.addWidget(self.image_diagnostics_btn)
            diagnostics_section.add_widget(diagnostics_group)
        ...
        def _connect_actions(self):
            ...
            self.image_diagnostics_btn.clicked.connect(self.start_image_diagnostics)
            ...
            self.image_diagnostics_action.triggered.connect(lambda checked=False: self.start_image_diagnostics())
            ...
            self.batch_preview_previous_btn.clicked.connect(self.show_previous_batch_preview)
            self.batch_preview_next_btn.clicked.connect(self.show_next_batch_preview)
            ...
            self.upper_canvas.profileMeasured.connect(self.receive_image_profile)
            self.lower_canvas.profileMeasured.connect(self.receive_image_profile)
